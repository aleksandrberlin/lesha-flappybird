...
| Человек-паук | 5 | висит на паутине от верхней границы экрана и одной дугой прочерчивает её сверху вниз |
| Дрон папарацци | 12 | подлетает и медленно подстраивается под твою высоту — от него не улететь по прямой |

**Бонусы — не враги.** Через экран проплывают Кэти Перри и лого Макдоналдса,
у обоих сердечко над головой; поймаешь — дополнительная жизнь (максимум 3).
Если жизни уже полные, вместо жизни дают +3 очка. Что именно вылетит — решает
монетка при спавне.
...
рамке-полароиде и подпись. Лёша выбирается **случайно**: примерно в 45% случаев
из тех, кого игрок ещё не находил, в остальных — из уже собранных, так что
коллекция набирается за много забегов, а не за один. Один и тот же Лёша никогда
не выпадает два раза подряд. Если Лёша новый, вместо «чекпоинта» появляется «НОВЫЙ ЛЁША!»,
играет фанфара и он добавляется в коллекцию.

Фотографии остаются фотографиями, а не пиксель-артом: рамка и текст вокруг
восьмибитные, само фото рисуется со сглаживанием. Канвас держит систему
координат 320x480, но рисует в буфер, кратный ей по плотности экрана
(`devicePixelRatio`), поэтому спрайты остаются чёткими квадратами, а фото
получают реальное разрешение, а не растягиваются вместе с пиксель-артом. У Лёши может быть несколько
кадров — тогда они крутятся как анимация (Шагающий Леша — пять кадров на 7 к/с,
Голодный — два кадра на 1.6 к/с). Дальше — тап, пробел или кнопка «дальше», после чего
...
крупно с подписью. Коллекция привязана к имени игрока и хранится только в Supabase
(таблица `flappy_lesha_finds`), поэтому с тем же именем находки подтянутся на
любом устройстве.

Сейчас в игре девять Лёш: Sassy, Голодный, Zoomed, Шагающий, Pop Mart,
Out-of-Office, Леша и бицепсы, Сплющенный, Assy. У каждого только имя —
подписей-описаний нет. В плитках коллекции показывается короткое имя
(`names`), на больших экранах — полное (`titles`).
Добавить своего:
...
GitHub Pages отдаёт всё с `cache-control: max-age=600`, причём `index.html` и
скрипты кэшируются независимо — свежая страница может подтянуть десятиминутной
давности `game.js` или, что хуже, старый `checkpoints-data.js` с другим набором
Лёш. Поэтому все ссылки в `index.html` помечены хэшем содержимого:

```html
<script src="src/game.js?v=be7b52ac"></script>
```

Изменился файл — изменился URL, и старая версия из кэша подтянуться не может;
файлы, которые не менялись, остаются закэшированными. Хэши проставляет скрипт:

```bash
python3 tools/stamp_version.py          # пересчитать и записать в index.html
python3 tools/stamp_version.py --check  # упасть, если штампы устарели
```

Его нужно прогнать после любой правки `src/*`, `css/*` или картинок — и перед
коммитом. Сборка для этого по-прежнему не нужна: раздача идёт прямо из ветки,
workflow не требуется. Если захочется, чтобы штамп проставлялся сам, это ровно
тот случай, когда Actions оправданы — тогда Pages переключается на
«Source: GitHub Actions», а workflow прогоняет скрипт и публикует результат.

## Превью ссылки

В `index.html` есть Open Graph и Twitter-теги, поэтому в Телеграме и прочих
чатах ссылка разворачивается в карточку с картинкой `assets/preview.png`
(1200x630). Карточка собирается из скриншота титульного экрана, игрового
шрифта и спрайтов:

```bash
python3 tools/make_preview.py путь/к/скриншоту.png
```

Адреса в тегах абсолютные (`https://flappylesha.ru/...`) — иначе чаты не
подтянут картинку. Телеграм кэширует превью: чтобы он перечитал теги после
деплоя, отправьте ссылку боту [@WebpageBot](https://t.me/WebpageBot).

## Загрузка

Перед первым экраном игра показывает полосу загрузки и **декодирует все фото
Лёш заранее** (`img.decode()`), а не по мере надобности. Поэтому чекпоинт
открывается мгновенно и найденный Лёша не появляется с задержкой. Если картинка
почему-то не загрузилась, через 12 секунд игра стартует всё равно — полоса
никогда не превращается в тупик. Диалог «Кто вы?» показывается уже после
загрузки.
...
Результат уходит на сервер **в момент смерти**, а не после анимации падения:
игрок, который закрывает вкладку сразу после столкновения, всё равно попадает
в рейтинг. Запрос помечен `keepalive`, поэтому переживает закрытие страницы, а
у каждого забега есть `run_id` — повтор запроса, который сервер уже принял, не
создаёт второй записи.

**Всё состояние игрока живёт на сервере.** Личный рекорд, место в рейтинге и
коллекция каждый раз запрашиваются у Supabase, локальных копий нет — забег
засчитан только тогда, когда его принял сервер. Единственное, что хранится в
браузере, — имя игрока (`localStorage`), чтобы не вводить его при каждом
заходе; сменить можно кнопкой «Имя» или клавишей `N`.

Без сети игра остаётся играбельной, но рейтинг покажет «нет связи с сервером»,
а на экране проигрыша вместо места будет та же пометка. Запросы профиля
повторяются несколько раз с нарастающей паузой, так что одна потерянная
попытка на мобильной сети не оставляет игрока с пустой коллекцией. Находки,
которые сервер не принял, досылаются при следующей синхронизации.
...
src/collection.js     коллекция находок игрока (Supabase)
