...
Каждые 10 очков игра замирает и показывает чекпоинт-экран: счёт, фотография в
рамке-полароиде и подпись. Фото идут по кругу — 1, 2, 1, 2, ... — и остаются
обычными фотографиями, а не пиксель-артом: рамка и текст вокруг восьмибитные,
само фото рисуется со сглаживанием. Дальше — тап, пробел или кнопка «дальше»,
после чего даётся полторы секунды неуязвимости, чтобы не влететь в бутылку
сразу после паузы.

Добавить своё фото:

```bash
# слот 3, квадратный кроп вокруг точки (cx, cy) со стороной side
python3 tools/make_checkpoints.py 3 фото.jpg --crop 1000,1150,1500
```

Скрипт кладёт кадр в `assets/checkpoints/3.jpg` (400x400, JPEG) и пересобирает
`src/checkpoints-data.js`, где фото лежат в base64 — поэтому игра работает и
с `file://`. После этого добавь слот в `order` в
[`src/checkpoints.js`](src/checkpoints.js); там же настраиваются подпись к
каждому фото, частота чекпоинтов и подбадривающие фразы.
...
src/checkpoints.js    какие фото и как часто показывать на чекпоинтах
...
assets/checkpoints/   фотографии для чекпоинт-экранов
